  ...
  public function InserisciCarrello($id_prodotto,$quantita){
    ...
    $id_utente=Session::get('user_id');
     //$querySelect='SELECT * FROM carrello WHERE id_utente="'.$utente.'" AND id_prodotto="'.$id_prodottoinput.'"';//
      ...
      if($row==null){
      Utente::find($id_utente)->carrello()->attach($id_prodotto,['quantita'=>$quantita]);
      }
...
if(count($errori)==0){
  ...
  //INSERISCO I PRODOTTI NELLA TABELLA ACQUISTO//
  foreach($prodottiDisponibili as $prodotto){
    $id_prodottoAcquistato=$prodotto->id;
    $quantitaAcquistata=$prodotto->pivot->quantita;
    Utente::find($id_utente)->acquisto()->attach($id_prodottoAcquistato,['quantita'=>$quantitaAcquistata]);
  }
  $errori=null;
}
